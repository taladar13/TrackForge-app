# TrackForge Partner Platform Plan
...
- 0.1 (2025-12-22): Initial draft for partner alignment and commercial proposal baselining.
...
- **Modular monolith** with clear domain boundaries and a stable API contract for both clients.
...
- **Auth**:
  - Access token (short) + refresh token rotation.
...
## Delivery phases (pragmatic rollout)

- **Phase A (foundation)**: org/roles, staff onboarding, client CRM core, import pipeline skeleton.
- **Phase B (core value)**: meal plan builder + assignment + mobile consumption/logging loop.
- **Phase C**: workout program builder + assignment + volume analytics.
- **Phase D**: medical rules + safety warnings in builders.
- **Phase E**: analytics dashboards + research exports.
- **Phase F**: multi-tenant hardening (RLS option, billing module later, messaging later).

## Key repos/files likely impacted when we implement

- Backend domains: `backend/app/domains/`
- Auth/permissions and dependencies: `backend/app/core/`
